...
router.post("/define", async (req, res) => {
  ...
  if (!word || !word.trim())
    return res
      .status(400)
      .json({ success: false, message: "Word missing hai boss! ✍️" });
  if (!userId)
    return res
      .status(400)
      .json({ success: false, message: "User session missing!" });

  let apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey)
    return res
      .status(500)
      .json({ success: false, message: "Server error: API Key missing." });
  ...
  const promptText = `
    You are an elite English vocabulary coach. Analyze the English input "${word.trim()}". It could be a word, phrase, idiom, or slang. Return ONLY a valid JSON object.
    Rules:
    1. "partOfSpeech": Give exact grammar category
    2. "meaning": Give short Hindi meaning in Devanagari
    3. "explanation": Explain the vibe/feel of this input in very simple, engaging Hinglish (max 3-4 lines). Explain EXACTLY in which contexts, moods, or situations the user should use it to sound like a native speaker. Clear all confusions.
    ...
    6. "sentences": Give EXACTLY 3 short, single-line sentences. The tone MUST be natural and conversational (like a deep late-night chat with friends), but the content should hit hard—incorporate interesting psychological facts, universal everyday truths, or mind-bending paradoxes. STRICTLY NO boring, robotic textbook language. Format exactly like this: "English sentence. (Hindi translation)" separated by \\n.
    `;
  ...
  try {
    ...
    return res
      .status(503)
      .json({ success: false, message: "AI Engine fail ho gaya!" });
  }
});
...
router.get("/history/:userId", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "DB History fetch nahi ho payi!" });
  }
});
  ...
  try {
    ...
    if (grade === "again") {
      ...
      if (item.interval === 0) {
        ...
        if (grade === "easy")
          item.interval = item.interval * item.easeFactor * 1.3;
      }
...
router.delete("/srs/clear-word-mistakes", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Mistakes clear nahi ho payi!" });
  }
});
...
router.get("/stats/:userId", async (req, res) => {
  try {
    ...
    if (!stats) {
      stats = await PracticeStats.create({
        userId,
        totalPracticed: 0,
        totalMistakes: 0,
      });
    ...
    return res
      .status(500)
      .json({ success: false, message: "Stats fetch fail ho gaye!" });
  }
});
...
router.post("/stats/update", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Stats update nahi ho paye!" });
  }
});
...
router.post("/generate-practice", async (req, res) => {
  ...
  if (!word)
    return res
      .status(400)
      .json({ success: false, message: "Target word missing hai! 🎯" });
  ...
  try {
    ...
    return res
      .status(503)
      .json({ success: false, message: "Game generate karne me dikkat aayi!" });
  }
});
  ...
  try {
    ...
    if (!word || !userId) {
      return res
        ...
        .json({
          success: false,
          message: "Missing Data: Word or userId nahi mila!",
        });
    }
    ...
    if (!updatedVocab) {
      return res
        .status(404)
        .json({ success: false, message: "History me ye word nahi mila!" });
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Database update fail ho gaya!" });
  }
});
...
router.post("/grammar-explain", async (req, res) => {
  ...
  if (!correctSentence || !userSentence) {
    return res
      .status(400)
      .json({ success: false, message: "Sentences missing." });
  }
  ...
  try {
    ...
    return res
      .status(503)
      .json({ success: false, message: "AI Engine busy hai boss!" });
  }
});
...
router.post("/speak", async (req, res) => {
  ...
  if (!text) {
    return res
      .status(400)
      .json({ success: false, message: "Text missing hai!" });
  }
  ...
  if (!GOOGLE_TTS_API_KEY) {
    return res
      ...
      .json({
        success: false,
        message: "TTS API Key missing! .env check karo.",
      });
  }
  ...
  try {
    ...
    return res
      ...
      .json({
        success: false,
        message: "Aawaz generate nahi ho payi! API Key check karo.",
      });
  }
});
...
router.post("/voice-chat", async (req, res) => {
  ...
  if (!message)
    return res
      .status(400)
      .json({ success: false, message: "Message zaroori hai" });

  let apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey)
    return res
      .status(500)
      .json({ success: false, message: "Gemini Key missing" });
  apiKey = String(apiKey)
    .replace(/[\r\n\t\s'"]/g, "")
    .trim();
  ...
  try {
    ...
    let aiReplyText = geminiRes.data.candidates[0].content.parts[0].text
      .trim()
      .replace(/[\*#_]/g, "");
...
router.post("/followup-chat", async (req, res) => {
  ...
  if (!userId || !word || !message) {
    return res
      .status(400)
      .json({ success: false, message: "Missing data for chat." });
  }
  ...
  try {
    ...
    try {
      const vocabDoc = await Vocab.findOne({
        userId,
    ...
    return res
      .status(503)
      .json({ success: false, message: "AI Teacher unavailable" });
  }
});
