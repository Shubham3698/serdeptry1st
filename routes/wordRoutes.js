...
const UserSRS = require("../models/english/UserSRS"); // 🔥 NAYA IMPORT SRS KE LIYE
...
router.get("/history/:userId", async (req, res) => {
  try {
    ...
    // 1. Fetch History using .lean() for faster, plain JS objects
    const userHistory = await Vocab.find({ userId }).lean().sort({ createdAt: -1 });
    
    // 2. Fetch all SRS Data for this user
    const userSrsData = await UserSRS.find({ userId }).lean();

    // 3. Merge SRS data into the history array
    const historyWithSRS = userHistory.map(wordItem => {
      // Find matching SRS doc for this specific word
      const srsMatch = userSrsData.find(srs => srs.word === wordItem.word);
      
      return {
        ...wordItem,
        srsData: srsMatch || null // Attach srsData so frontend can filter "dueCards"
      };
    });

    return res.json({ success: true, data: historyWithSRS });
  } catch (error) {
